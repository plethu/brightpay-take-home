...
function findQuantityInput(stepButton) {
    const form = stepButton.closest("form");
    return form?.querySelector("input[name='quantity']") ?? null;
}
function updateQuantityDisabledState(input) {
    const control = input.closest(".qty-control");
    const decrement = control?.querySelector("[data-qty-step='-1']");
    if (!decrement) {
        return;
    }
...
function pulse(element) {
    ...
    element.animate([{ transform: "scale(1)" }, { transform: "scale(1.12)" }, { transform: "scale(1)" }], { duration: 240, easing: EASE }).onfinish = () => {
        if (element instanceof HTMLElement) {
            delete element.dataset.pulse;
        }
    };
}
function animateLineIn(line) {
    ...
    ], { duration: 240, easing: EASE }).onfinish = () => {
        if (line instanceof HTMLElement) {
            line.style.overflow = "";
...
function captureLineStates(root = document) {
...
function animateOfferOut(line, previous) {
    if (reduceMotion()) {
        return;
    }
...
function collapseAndRemove(element) {
    ...
    ], { duration: 200, easing: EASE }).onfinish = () => element.remove();
}
function animateLineOut(line) {
    if (reduceMotion()) {
        line.remove();
        return;
    }
    ...
    ], { duration: 220, easing: EASE }).onfinish = () => line.remove();
}
function animateRemovedLines(records) {
...
function processLineChanges(root = document) {
    root.querySelectorAll(".sale-line[data-sku]").forEach((line) => {
        const sku = line.getAttribute("data-sku");
        if (!sku) {
            return;
...
function observeCheckoutChanges(root) {
...
function submitAfterLineOut(button) {
    const form = button.form;
    const line = button.closest(".sale-line");
    const quantity = line?.querySelector(".line-qty")?.textContent?.trim();
    ...
    ], { duration: 180, easing: EASE }).onfinish = () => form.requestSubmit();
    return true;
}
...
function wireQuantityInputs(root, signal) {
...
function replayQueryFeedback(root) {
...
export function pulseToast() {
    ...
    toast.animate([{ transform: "scale(1)" }, { transform: "scale(1.04)" }, { transform: "scale(1)" }], { duration: 200, easing: EASE });
}
export function initialize() {
    ...
    root.querySelectorAll("[data-clear-form]").forEach(wireClearDialog);
